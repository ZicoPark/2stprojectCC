			...
			<section class="content">
				<form action="freeUpdate.do" method="post"
					...
					<br> <input type="text" name="member_id"
						value="${sessionScope.id}" readonly="readonly" hidden="true"
						required /> <input type="text" name="id" value="${detailms.id}"
						readonly="readonly" hidden="true" required /> 문서번호 ${detailms.id}
					<div class="row">
						<div class="col-6">
							<div>
								<input class="form-control" name="subject" id="subject"
									value=${detailms.subject } maxlength="19"
									onkeyup="counter(event, '20')" /> <span id="reCount">0 /
									20</span>
							</div>
					...
					<div class="custom-file">
						...
						<label class="custom-file-label" for="fileInput"> 첨부파일을
							선택하세요. </label>
					</div>

					<!--  파일 선택 버튼 숨겼음 !! 왜냐면 file 요소 떄문에 원래 있던 애들 사라짐 -->
					<input type="file" name="attachment" multiple="multiple"
						onchange="displayFileNames(event)" id="fileInput"
						style="display: none;">
					...
					<div id="fileNames">
						<c:if test="${detailfile.size() > 0 }">
							<c:forEach items="${detailfile}" var="i">
								<div>
									<input type="text" name="removeFile" value="${i.id}"
										hidden="true" />${i.ori_file_name} <i
										class="fas fa-times cancel-icon"
										onclick="removeFileEvent(this)"></i>
								</div>
							</c:forEach>
						</c:if>
					</div>
